...
# Setup logging
logging.basicConfig(
    filename="king_analysis.log", level=logging.INFO, format="%(message)s"
)
...
if __name__ == "__main__":
    ...
    if not (firstkin > secondkin) & (firstkin > thirdkin):
        print(
            "Wrong values enter for 1st degree Estimate. 1st degree Estimate cannot be smaller than Second degree and third degree"
        )
    elif not (secondkin > thirdkin):
        print(
            "Wrong values enter for 2nd degree Estimate. Second degree Estimate cannot be smaller than third degree"
        )
    ...
    combined_kinship, kinship_history = analyzer.incremental_analysis(
        data_A, data_B, snps, iterations
    )
    ...
    final_estimates_df = pd.DataFrame(
        list(final_estimates.items()), columns=["Pair", "Degree"]
    )
