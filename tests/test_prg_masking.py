...
def test_prg_masking_correctness():
    ...
    assert np.all(
        array_error < 1e-10
    ), f"Array aggregation error too large: {array_error}"
...
def test_privacy_properties():
    ...
    for i, helper in enumerate(client_helpers):
        ...
        print(
            f"Client {i}: original {sensitive_seeds[i]} -> masked {masked_seed:.2f} (diff: {difference:.2f})"
        )
        assert (
            difference > 1000
        ), f"Masking difference too small for client {i}: {difference}"
    ...
    print(
        f"Masked values range: {min(masked_values_normalized):.2f} - {max(masked_values_normalized):.2f}"
    )
    ...
    print(
        f"Minimum distance from any masked to any original: {min_distance_to_any_original:.2f}"
    )
    assert (
        min_distance_to_any_original > 100
    ), "Some masked values too close to originals"
...
def test_multiple_rounds():
    ...
    for round_num in range(num_rounds):
        ...
        print(
            f"Round {round_num + 1}: expected {expected_sum}, got {aggregated_value:.2f}, error {error:.2f}"
        )
