...
# Add the parent directory to the path to import the package when tests are run directly.
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from pipeline.simulation.simulated_data.generate_synthetic_data import SyntheticDataGenerator
...
class TestSyntheticDataGenerator:
    ...
    def test_kinship_coefficients(self, generator, small_config):
        ...
        Gdata_full, rel_ids_df = generator.create_relatives(
            unrelated_genotypes, n=n_relatives
        )
        ...
        for relationship_type, (min_coeff, max_coeff) in kinship_ranges.items():
            ...
            for _, row in rel_rows.iterrows():
                ...
                if i is not None and j is not None:
                    coeff = generator.calculate_king_kinship(
                        genotypes[i, :100], genotypes[j, :100]
                    )
                    kinship_coeffs.append(coeff)
            if kinship_coeffs:
                mean_kinship = np.mean(kinship_coeffs)
                print(
                    f"{relationship_type}: mean kinship = {mean_kinship:.3f} (expected range: {min_coeff:.3f}-{max_coeff:.3f})"
                )
                # Allow for more stochasticity in mean kinship
                assert (
                    min_coeff * 0.7 <= mean_kinship <= max_coeff * 1.3
                ...
                for coeff in kinship_coeffs:
                    assert (
                        0 <= coeff <= 1
                    ), f"Kinship coefficient {coeff} not in [0,1] range"
    ...
    def test_generate_genotypes(self, generator, tiny_config):
        ...
        family_info = generator.generate_family_structure(
            n_samples,
            tiny_config["relative_fraction"],
            tiny_config["relative_distribution"],
        )
        genotypes_with_family = generator.generate_genotypes(
            n_samples, n_snps, mafs, family_info
        )
    ...
    def test_introduce_missingness(self, generator, tiny_config):
        ...
        genotypes_with_missing = generator.introduce_missingness(
            genotypes, missing_rate
        )
    ...
    def test_generate_binary_phenotype(self, generator, tiny_config):
        ...
        phenotypes, causal_snps = generator.generate_binary_phenotype(
            genotypes, causal_fraction
        )
    ...
    def test_generate_scale_without_seed_uses_module_numpy(self, generator, monkeypatch):
        """Regression test for seed=None path using the module-level numpy import."""
        generator.scales["test_seedless"] = {
            "samples": 10,
            "snps": 5,
            "centers": 2,
            "causal_fraction": 0.2,
            ...
            "relative_distribution": {"first": 1.0, "second": 0.0, "third": 0.0},
        }

        def fake_create_relatives(unrelated_genotypes, n):
            n_snps, n_unrelated = unrelated_genotypes.shape
            sample_ids = [f"sample_{idx}" for idx in range(n_unrelated + n)]
            genotypes = np.zeros((n_snps, n_unrelated + n), dtype=int)
            relatives = pd.DataFrame(
                {
                    "SampleID": sample_ids[n_unrelated:],
                    "AncestryID": sample_ids[:n],
                    "Relatedness": ["first-degree"] * n,
                }
            )
            return pd.DataFrame(genotypes, columns=sample_ids), relatives

        def fake_create_plink_files(genotypes, phenotypes, mafs, scale_name, family_info):
            scale_dir = Path(generator.output_dir) / scale_name
            scale_dir.mkdir(parents=True, exist_ok=True)
            return scale_dir

        monkeypatch.setattr(generator, "create_relatives", fake_create_relatives)
        monkeypatch.setattr(generator, "create_plink_files", fake_create_plink_files)
        monkeypatch.setattr(generator, "partition_samples", lambda *args, **kwargs: None)

        scale_dir = generator.generate_scale("test_seedless", "even", seed=None)

        assert scale_dir.name == "test_seedless"
    ...
    def test_even_partition(self, generator, tiny_config):
        ...
        sample_indices_per_center = generator._even_partition(
            n_samples, n_centers, phenotypes
        )
    ...
    def test_uneven_partition(self, generator, small_config):
        ...
        sample_indices_per_center = generator._uneven_partition(
            n_samples, n_centers, phenotypes
        )
    ...
    def test_skewed_partition(self, generator, small_config):
        ...
        sample_indices_per_center = generator._skewed_partition(
            n_samples, n_centers, phenotypes
        )
    ...
    def test_create_plink_files(self, generator, temp_dir, tiny_config):
        ...
        # Create PLINK files
        scale_dir = generator.create_plink_files(
            genotypes, phenotypes, mafs, "test_scale", family_info
        )
