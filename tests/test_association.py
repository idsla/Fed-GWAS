...
def main():
    ...
    selected_indices = input(
        "Enter the number of the functions to run e.g 1 3 for the first and third function "
    ).split()
    ...
    association = GWASAssociation(
        bed_path=bed_path, bim_path=bim_path, fam_path=fam_path, threshold=threshold
    )
