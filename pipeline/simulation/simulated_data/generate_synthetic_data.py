...
- Small: 2,000 samples, 20,000 SNPs, 2 centers
- Medium: 5,000 samples, 50,000 SNPs, 2 centers
- Large: UK Biobank-scale proxy (see --scale large)
...
class SyntheticDataGenerator:
    def _find_plink(self):
        ...
        # Determine project root (assuming this file is in pipeline/simulation/simulated_data/)
        ...
        import logging
        logging.getLogger(__name__).warning(f"PLINK not found in expected location: {plink_path}")
        return str(plink_path)
    ...
    def __init__(self, output_dir="simulated_data"):
        ...
        self.scales = {
            ...
            # Matpool appendix scales (see experiments/performance/scales.yaml)
            'small': {
                'samples': 2000,
                'snps': 20000,
                'centers': 2,
    ...
    def generate_family_structure(self, n_samples, relative_fraction=0.1, relative_distribution=None):
        ...
        NOTE: This method is NOT used in the main generation pipeline.
        It is kept for testing purposes only. The main pipeline uses create_relatives().
    ...
    def create_plink_files(self, genotypes, phenotypes, mafs, scale_name, family_info=None):
        ...
        for i in range(n_samples):
            ...
            # Convert binary phenotype to PLINK format: 0->1 (control), 1->2 (case)
            plink_phenotype = phenotypes[i] + 1 if phenotypes[i] != -9 else -9
            fam_data.append([f"F{i+1:06d}", f"I{i+1:06d}", father_id, mother_id, "1", plink_phenotype])
    ...
    def _even_partition(self, n_samples, n_centers, phenotypes):
        ...
        total_controls = len(control_indices)
        
        # Calculate target cases and controls per center to maintain overall ratio
        total_case_ratio = total_cases / n_samples
        
        sample_indices_per_center = []
        for center in range(n_centers):
            center_samples = samples_per_center + (1 if center < remainder else 0)
            
            # Calculate target cases for this center to maintain overall ratio
            target_cases = int(center_samples * total_case_ratio)
            target_controls = center_samples - target_cases
            
            center_cases = []
            center_controls = []
            
            # Assign cases
            cases_to_assign = min(target_cases, len(case_indices))
            for _ in range(cases_to_assign):
                center_cases.append(case_indices.pop())
            
            # Assign controls
            controls_to_assign = min(target_controls, len(control_indices))
            for _ in range(controls_to_assign):
                center_controls.append(control_indices.pop())
            
            # If we still need more samples, fill with remaining cases or controls
            remaining_needed = center_samples - len(center_cases) - len(center_controls)
    ...
    def _uneven_partition(self, n_samples, n_centers, phenotypes):
        """Uneven sample sizes with balanced phenotype distribution (similar case ratios across centers)."""
        if n_centers == 2:
            ratios = [0.6, 0.4]
        elif n_centers == 3:
            ratios = [0.5, 0.3, 0.2]
        ...
        # Calculate overall case ratio
        overall_case_ratio = np.mean(phenotypes)
        ...
        for center in range(n_centers):
            ...
            # Maintain overall case ratio for balanced phenotype
            cases_needed = int(center_samples * overall_case_ratio)
            controls_needed = center_samples - cases_needed
            
            # Ensure minimum cases (at least 1 case per center)
            min_cases = max(1, int(center_samples * 0.1))  # At least 10% cases
            cases_needed = max(cases_needed, min_cases)
            controls_needed = center_samples - cases_needed
        ...
        for center, indices in enumerate(sample_indices_per_center):
            ...
            print(f"[Uneven Partition] Center {center+1}: {len(indices)} samples, case ratio: {case_ratio:.3f}")
        
        return sample_indices_per_center

    def _skewed_partition(self, n_samples, n_centers, phenotypes):
        """Uneven sample sizes with phenotype skew (different case ratios across centers)."""
        if n_centers == 3:
            size_ratios = [0.6, 0.3, 0.1]
            case_ratios = [0.5, 0.3, 0.15]  # Increased minimum from 0.1 to 0.15
        elif n_centers == 5:
            size_ratios = [0.4, 0.25, 0.2, 0.1, 0.05]
            case_ratios = [0.5, 0.4, 0.3, 0.2, 0.15]  # Increased minimum from 0.1 to 0.15
        elif n_centers == 7:
        ...
        # Pre-calculate total cases needed to ensure we don't run out
        total_cases_needed = 0
        for center in range(n_centers):
            center_samples = int(n_samples * size_ratios[center])
            if center == n_centers - 1:
                center_samples = n_samples - sum(int(n_samples * size_ratios[i]) for i in range(n_centers - 1))
            min_cases = max(1, int(center_samples * 0.1))
            target_cases = max(min_cases, int(center_samples * case_ratios[center]))
            total_cases_needed += target_cases
        
        # If we don't have enough cases, adjust the ratios to be more conservative
        available_cases = len(case_indices)
        if total_cases_needed > available_cases:
            print(f"[Skewed Partition] Warning: Need {total_cases_needed} cases but only have {available_cases}. Adjusting ratios.")
            # Scale down case ratios proportionally while maintaining minimum 1 case per center
            scale_factor = max(1, available_cases) / total_cases_needed
            for i in range(n_centers):
                center_samples = int(n_samples * size_ratios[i])
                if i == n_centers - 1:
                    center_samples = n_samples - sum(int(n_samples * size_ratios[j]) for j in range(n_centers - 1))
                min_cases = max(1, int(center_samples * 0.1))
                adjusted_cases = max(min_cases, int(center_samples * case_ratios[i] * scale_factor))
                case_ratios[i] = adjusted_cases / center_samples
        ...
        for center in range(n_centers):
            ...
            # Calculate cases needed with adjusted ratio
            target_case_ratio = case_ratios[center]
            cases_needed = int(center_samples * target_case_ratio)
            controls_needed = center_samples - cases_needed
            ...
            available_cases = len(case_indices)
            if cases_needed > available_cases:
                cases_needed = max(1, min(available_cases, center_samples // 2))
                controls_needed = center_samples - cases_needed
        ...
        for center, indices in enumerate(sample_indices_per_center):
            center_phenotypes = phenotypes[indices]
            case_ratio = np.mean(center_phenotypes)
            print(f"[Skewed Partition] Center {center+1}: {len(indices)} samples, case ratio: {case_ratio:.3f}")
    ...
    def generate_scale(self, scale_name, partition_strategy="even", seed=None):
        ...
        logger.info(f"Configuration: {n_samples} samples, {n_snps} SNPs, {n_centers} centers")
        logger.info(f"Family structure: {n_unrelated} unrelated, {n_relatives} relatives")
        ...
        # Vectorized generation of all genotypes at once
        unrelated_genotypes = np.random.binomial(2, mafs[:, np.newaxis], size=(n_snps, n_unrelated))
    ...
    def add_child(self, df, childID, fatherID, motherID):
        ...
        # Vectorized Mendelian inheritance
        # Generate random probabilities for all SNPs at once
        p = np.random.uniform(0, 1, len(fatherCol))
        
        # Initialize child column
        childCol = np.zeros_like(fatherCol)
        
        # Vectorized inheritance rules
        # Case 1: Both parents homozygous reference (0,0) -> child homozygous reference (0)
        mask = (fatherCol == 0) & (motherCol == 0)
        childCol[mask] = 0
        
        # Case 2: One parent heterozygous, one homozygous reference (0,1) or (1,0) -> 50% chance of each
        mask = ((fatherCol == 0) & (motherCol == 1)) | ((fatherCol == 1) & (motherCol == 0))
        childCol[mask] = np.where(p[mask] > 0.5, 1, 0)
        
        # Case 3: One parent homozygous reference, one homozygous alternate (0,2) or (2,0) -> heterozygous (1)
        mask = ((fatherCol == 0) & (motherCol == 2)) | ((fatherCol == 2) & (motherCol == 0))
        childCol[mask] = 1
        
        # Case 4: Both parents heterozygous (1,1) -> 1/3 chance of each genotype
        mask = (fatherCol == 1) & (motherCol == 1)
        childCol[mask] = np.where(p[mask] < 0.333, 0, np.where(p[mask] < 0.666, 1, 2))
        
        # Case 5: One parent heterozygous, one homozygous alternate (1,2) or (2,1) -> 50% chance of heterozygous or homozygous alternate
        mask = ((fatherCol == 1) & (motherCol == 2)) | ((fatherCol == 2) & (motherCol == 1))
        childCol[mask] = np.where(p[mask] > 0.5, 2, 1)
        
        # Case 6: Both parents homozygous alternate (2,2) -> child homozygous alternate (2)
        mask = (fatherCol == 2) & (motherCol == 2)
        childCol[mask] = 2
    ...
    def create_relatives(self, Gdata, n=40):
        ...
        # Pre-allocate all child IDs and parent pairs
        parent_ids = np.random.choice(Gdata_n.columns, size=n, replace=False)
        
        # Step 1: 1st generation
        n_first = n // 2
        F_id = parent_ids[:n_first]
        M_id = parent_ids[n_first:2*n_first]
        child_ids_1C = [f'1C_{i}' for i in range(n_first)]
        
        # Batch create first generation children
        logger.info(f"Creating {n_first} first-generation relatives...")
        for i, (c_id, f_id, m_id) in enumerate(zip(child_ids_1C, F_id, M_id)):
            if i % 1000 == 0 and i > 0:  # Log progress every 1000 relatives
                logger.info(f"Created {i}/{n_first} first-generation relatives...")
            Gdata_n = self.add_child(Gdata_n, c_id, f_id, m_id)
            rel_ids.extend([[c_id, f_id, 'first-degree'], [c_id, m_id, 'first-degree']])
        
        # Step 2: 2nd generation
        n_second = n // 4
        if n_second > 0 and len(child_ids_1C) >= 2:
            logger.info(f"Creating {n_second} second-generation relatives...")
            np.random.shuffle(child_ids_1C)
            group_1CF = child_ids_1C[:n_second]
            group_1CM = child_ids_1C[n_second:2*n_second]
            child_ids_2C = [f'2C_{i}' for i in range(n_second)]
            
            # Batch create second generation children
            for i, (c_id, f_id, m_id) in enumerate(zip(child_ids_2C, group_1CF, group_1CM)):
                if i % 1000 == 0 and i > 0:  # Log progress every 1000 relatives
                    logger.info(f"Created {i}/{n_second} second-generation relatives...")
                Gdata_n = self.add_child(Gdata_n, c_id, f_id, m_id)
                # Add relationships to grandparents
                for grandparent in [f_id, m_id]:
                    if grandparent in Gdata_n.columns:
                        rel_ids.append([c_id, grandparent, 'second-degree'])
        
        # Step 3: 3rd generation
        n_third = n // 8
        if n_third > 0 and len(child_ids_2C) >= 2:
            logger.info(f"Creating {n_third} third-generation relatives...")
            np.random.shuffle(child_ids_2C)
            group_2CF = child_ids_2C[:n_third]
            group_2CM = child_ids_2C[n_third:2*n_third]
            child_ids_3C = [f'3C_{i}' for i in range(n_third)]
            
            # Batch create third generation children
            for i, (c_id, f_id, m_id) in enumerate(zip(child_ids_3C, group_2CF, group_2CM)):
                if i % 1000 == 0 and i > 0:  # Log progress every 1000 relatives
                    logger.info(f"Created {i}/{n_third} third-generation relatives...")
                Gdata_n = self.add_child(Gdata_n, c_id, f_id, m_id)
                # Add relationships to great-grandparents
                for great_grandparent in [f_id, m_id]:
                    if great_grandparent in Gdata_n.columns:
                        rel_ids.append([c_id, great_grandparent, 'third-degree'])
        
        logger.info(f"Completed creating {len(rel_ids)} relative relationships")
